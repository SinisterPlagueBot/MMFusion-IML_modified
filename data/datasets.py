...
from albumentations import Compose, RandomScale, PadIfNeeded, RandomCrop, ImageCompression, LongestMaxSize
    ...
    def __getitem__(self, index):
        ...
        if mask_path == 'None':
            ...
            with cwd(self.base_path):
                ...
                if mask is None:
                    print(f"Failed to read the mask: {mask_path}")
                    raise ValueError(f"Failed to read the mask: {mask_path}")
        ...
        if self.train:
            ...
            res = LongestMaxSize(max_size=2048)(image=image, mask=mask)
            image = res['image']
            mask = res['mask']
